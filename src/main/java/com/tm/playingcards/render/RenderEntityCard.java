...
public class RenderEntityCard extends EntityRenderer<EntityCard> {
    ...
    @Override
    protected boolean shouldShowName(EntityCard entity) {
        return Minecraft.getInstance().player.isCrouching() && entity.getStackSize() > 1;
    }

    @Override
    protected void renderNameTag(EntityCard entity, Component component, PoseStack poseStack, MultiBufferSource buffer, int combinedLight) {
